...
function hz_yisheng(id) {
	...
	$.ajax({
		...
		data : {
			beizhu : $("#beizhu").val(),
			user_huanzhe : $("#user_huanzhe").val()

		},
		success : function(data) {

			var yuyueyisheng = Boolean(json[0].yuyueyisheng);
			...
			// 编写font标签代码
			var fonthtml = "<font color =" + fontcolor + " >" + message
					+ "</font>";
			// 在 usernamespan 标签中添加 font 标签
			$("#amigespan").html(fonthtml);
		}
	...
	//
	// for ( var i in json) {
	// str += "<table border='0px' width='100%' align='center'><tr
	// width='100%'>"
